...
The interface assumes the Shiny app file [app.R](https://github.com/byandell/qtl2shiny/blob/master/inst/qtl2shinyApp/app.R) is in a directory with other support documents and the data. There is no need to change this file; just put it in place.
...
The `about.md` and `about-extended.md` files are place-holders for now for online documentation. All data may be stored under the directory (`qtl2shinyData`).
...
```{r eval=FALSE}
project_info <- read.csv("project.csv")
...
Genotype probabilities get stored in `feather` databases by chromosome for rapid access. This takes a few steps, starting with creating a directory to store files and calculating genotype probabilities for allele pairs. 

```{r eval=FALSE}
# Create directory for feather data; must call it "genoprob"
feather_dir <- file.path(project_dir, "genoprob")
dir.create(feather_dir)

# Calculate allele pair genotype probabilities
probs <- calc_genoprob(recla, gmap, err=0.002)
# Convert allele pair probs to feather via qtl2feather
fprobs <- feather_genoprob(probs, "probs", feather_dir)
...
# Convert allele probs to feather via qtl2feather
faprobs <- feather_genoprob(aprobs, "aprobs", feather_dir)
...
```{r eval=FALSE}
query_mrna <- DOread::create_mrna_query_func_do(NULL)
...
Genotype probabilities are assumed to be `feather` databases.
